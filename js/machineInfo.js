...
if (typeof comp !== 'undefined') {
	if (comp.browser === 'IE' || comp.browser === 'Safari') {
		version = 'b-version-' + Array(parseInt(comp.bversion) + 1).join('i') + ' ';
	}
	if (comp.device !== '') {
		device = ' d-' + comp.device.toLowerCase();
	}
	if (comp.os !== '') {
		os = ' os-' + comp.os.toLowerCase();
	}
}
var isMobile = comp.device.toLowerCase() !== '';
...
function getOSInfo() {
	if (navigator.appVersion.match(/Win/i)) {
		...
		return 'iOS';
	} else if (navigator.appVersion.match(/X11/i)) {
		return 'Unix';
