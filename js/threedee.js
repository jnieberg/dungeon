...
function tdReloadView() {
    var canvas = document.getElementById('view');
    canvas.width  = window.innerWidth - viewSize * squareSize - 50;
    canvas.height = window.innerHeight - 70;
    renderer.setViewport(0, 0, canvas.width, canvas.height);
    renderer.setSize( canvas.width, canvas.height );
    camera.fov = 45;
    ...
    (function(force) {
        setTimeout(function() {
            if(typeof force !== "undefined" && force) {
                ...
                var emissive = (rand(Math.floor(origin.f / 10), 0, 0, 112.10, 8) * 4 + 16);
                emissive += ',' + (rand(Math.floor(origin.f / 10), 0, 0, 843.59, 8) * 4 + 16);
                emissive += ',' + (rand(Math.floor(origin.f / 10), 0, 0, 650.22, 8) * 4 + 16);
                ...
                tdUpdateCameraLight();
                //tdGenerateTexture(Math.floor(origin.f / 10));
            }
            var i = 0;
            for(var x = -Math.floor(mapSize / 2); x < Math.floor(mapSize / 2); x++) {
                for(var y = -Math.floor(mapSize / 2); y < Math.floor(mapSize / 2); y++) {
                    if(x >= -Math.floor(tdViewSize / 2) - 1 && x <= Math.floor(tdViewSize / 2) + 1 && y >= -Math.floor(tdViewSize / 2) - 1 && y <= Math.floor(tdViewSize / 2) + 1) {
                        //if(typeof force !== "undefined" || force || typeof scene.getObjectByName(keyLocation(origin.f, origin.x + x, origin.y + y)) === "undefined") {
                        if((typeof force !== "undefined" && force) || typeof map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh === "undefined" || map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh === null) {
                            tdDraw(origin.f, origin.x + x, origin.y + y);
                            i++;
                        }
                        map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh.visible = true;
                    } else if(typeof map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh !== "undefined" && map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh !== null) {
                        map[Math.floor(mapSize / 2) + x][Math.floor(mapSize / 2) + y].mesh.visible = false;
                    }
                }
            }
...
function tdCreateObject(f, x, y) {
    ...
    for(var o = 0; o < ob.length; o++) {
        ...
        switch(ob[o].replace(/[0-9]/g, '')) {
            ...
            case 'ceil-light': x1 = 0.4, y1 = 0.4, z1 = 1.5, x2 = 0.2, y2 = 0.2, z2 = 0.5; type = 'ceil-light'; mat = 'ceil-light'; rnd = 100; seed = 129.22; break;
            case 'wall-light': x1 = 0.45, y1 = 1.001, z1 = 0.7, x2 = 0.1, y2 = 0.15, z2 = 0.2; type = 'wall-light'; mat = 'wall-light'; break;
            case 'wall-light-high': x1 = 0.45, y1 = 1.001, z1 = 1.7, x2 = 0.1, y2 = 0.15, z2 = 0.2; type = 'wall-light-high'; mat = 'wall-light'; break;
            case 'window-high': x1 = 0, y1 = 0.49, z1 = 1, x2 = 1, y2 = 0.02, z2 = 1; type = 'window'; mat = 'window'; seed = 812.93; break;
            case 'rune': x1 = 0, y1 = 0, z1 = 0.001, x2 = 1, y2 = 1, z2 = 1; type = 'floor-deco'; mat = 'rune'; break;
            default: break;
        }
        ...
        if(o === 0) {
            if(getSquareFeature(x, y, 'double') === 'ceil') {
                ms1 = tdDrawObject('floor-ceil-double', msg, f, x, y, 0, 0, 1, 1, 1, 1, 0, 'floor-ceil', 'floor', 100);
                if(ms1 !== null) {
                    tdRotateInWorld('y', ms1, (-(d + 2) * 90) * Math.PI / 180);
                }
            } else if(getSquareFeature(x, y, 'double') === 'wall') {
                ms2 = tdDrawObject('wall', msg, f, x, y, 0, 0, 1, 1, 1, 1, 0, 'wall', 'wall', 100, 129.22);
                if(ms2 !== null) {
                    tdRotateInWorld('y', ms2, (-(d + 2) * 90) * Math.PI / 180);
                }
            }
            if(getSquareFeature(x, y, 'triple') === 'wall') {
                ms3 = tdDrawObject('wall', msg, f + 1, x, y, 0, 0, 2, 1, 1, 1, 0, 'wall', 'wall', 100, 129.22);
                if(ms3 !== null) {
...
function tdDrawObject(type, msg, f, x, y, x1, y1, z1, x2, y2, z2, d, metatype, mat, rnd, seed) {
    ...
    if(metatype !== '') {
        switch(metatype) {
            ...
            case 'pillar2':
            ms = new THREE.Object3D();
            var r = rand(Math.floor(f / Math.ceil(rnd / 10)), Math.floor(x / rnd), Math.floor(y / rnd), 529.52, 5);
            if(r === 0) {
            ...
            if(mat !== '') {
                if(typeof tdMaterial[mat].light !== "undefined" && tdMaterial[mat].light) {
                    ...
                    if(tdMaterial[mat].lightColor === 'random') {
                        var h = rand(Math.floor(f / 10), 0, 0, 112.11, 100) * 0.01;
                        var s = 1.0;
                        ...
                        var h = rand(f, x, y, 441.97, 100) * 0.01;
                        var s = 1.0;
...
function tdBlurGeometry(g) {
    ...
    for(var v in g.vertices) {
        ...
        g.vertices[v].x += rand(origin.f, g.id, 0, 212.88 * v, 100) * 0.0004 - 0.02;
        g.vertices[v].y += rand(origin.f, g.id, 0, 763.99 * v, 100) * 0.0004 - 0.02;
        g.vertices[v].z += rand(origin.f, g.id, 0, 30.71 * v, 100) * 0.0002;
    }
